...
const musicoalbumValidationRules = {

    musicoId:(musicoId)=>{

        let mensagens = [];
        if(!musicoId || musicoId.trim().length === 0){ //o trim remove todos os espaços em branco e o lenght é o tamanho
            mensagens.push('Obrigatório informar o Id do músico relacionado ao álbum');
        }
        return mensagens;
    },
...
export const useValidarDadosMusicoAlbum = () => {
    return useValidator(MUSICOALBUM, ERROR_MUSICOALBUM, musicoalbumValidationRules);
}